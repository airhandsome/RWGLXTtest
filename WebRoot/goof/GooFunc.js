...
//初始化区域图的对象
	...
	if(!property.haveTool){
		// this.$bgDiv.append("<div class='GooFlow_tool'"+(property.haveHead? "":" style='margin-top:3px'")+"><div style='height:"+(height-headHeight-(property.haveHead? 7:10))+"px' class='GooFlow_tool_div'></div></div>");
		this.$bgDiv.append("<div class='GooFlow_tool'"+(property.haveHead? "":" style='margin-top:3px'")+"></div>");
		 this.$tool=this.$bgDiv.find(".GooFlow_tool div");
		toolWidth=5;
		this.$nowType="cursor";
		this.$editable=false;//只有具有工具栏时可编辑
	}
	...
	if(!property.haveTool&&(!property.isExe)){//timearea and tag
		tmp = "<div style='background-image:url(images/gooflow_blank.gif);width:2270px;height:20px'><div class='timearea'></div>";
		var num;
	    tmp=tmp+"<div class='tag' ><em></em><span></span><div > 2:2:2</div></div>";
	    for(num=0;num<6;num++) //num代表有多少个固定的时间点
		  	tmp=tmp+"<div class='location'  id='lo"+num+"' ><em></em><div>0:0:0</div></div>";

		tmp=tmp+"</div>";
		this.$test = $(tmp);
		this.$bgDiv.children(".GooFlow_work").append(this.$test);
	}
	...
	if(this.$editable){
	  ...
	  this.$selectArea=$("<select><option>true</option><option  selected='selected'>false</option><option> </option></select>");
	  this.$bgDiv.append(this.$selectArea);
	  ...
	  //格式function(id，type,width,height)：id是单元的唯一标识ID,type是单元的种类,有"node","line","area"三种取值;width是新的宽度,addareaheight是新的高度
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
			...
			var t=getElCoordinate(This.$workArea[0]);
			 var fromNodeId = This.$lineData[this.id].from; //得到源结点的id值
//			 var toNodeId = This.$lineData[this.id].to;
			 var  fromNodeType = This.$nodeData[fromNodeId].type;//得到源结点的类型
//			 var toNodeType = This.$nodeData[toNodeId].type;
			

			 if(fromNodeType == "fork"){//只对fork结点处理
					This.$selectArea.css({display:"block",width:120, 
					left:t.left+x-This.$workArea[0].parentNode.scrollLeft,
					top:t.top+y-This.$workArea[0].parentNode.scrollTop}).data("id",This.$focus).focus();//append("<option>"+oldTxt+"</option></select>").
			 		}
//			 else if(fromNodeType == "task"){			 		
//			 	This.$textArea.val(oldTxt).css({display:"block",width:120,height:14, //文本框改下拉框!@#
//			    left:t.left+x-This.$workArea[0].parentNode.scrollLeft,
//		    	top:t.top+y-This.$workArea[0].parentNode.scrollTop}).data("id",This.$focus).focus();
//			 	}
			This.$workArea.parent().one("mousedown",function(e){
				if(e.button==2)return false;
				if(fromNodeType == "fork"){//修改
					This.setName(This.$selectArea.data("id"),This.$selectArea.val(),"line");
					This.$selectArea.val("").removeData("id").hide();
				}
//				else if(fromNodeType == "task"){
...
//				}
	...
	initWorkForNode:function(){//!@#0329
		...
		this.$workArea.delegate(".GooFlow_item","click",{inthis:this},function(e){
            ...
            if(data.length == 2)
                $("#demo").attr("data-focus", $(this).attr("id") + '#' + data[1]);
            else
                $("#demo").attr("data-focus", $(this).attr("id"));
			//alert($(this).text());
			//This.loadMonitor(123);//
			This.getNodesId(1);//
		     $("[data-toggle='popover']").popover({html:true});//
		 	//setInterval(This.loadDataAjax, 2000);//
		...
		//绑定双击编辑事件
//		this.$workArea.delegate(".GooFlow_item","dblclick",{inthis:this},function(e){  //双击node时，会判断这个node的#后面内容 然后通过json传到DB，更新session  
//			 var nodename = $(this).attr("id");
//		     var style = $(this).attr("style");
////			 var NodeId = This.$nodeData[e.data.inthis.$id].from; //得到源结点的id值
////			 var  NodeType = This.$nodeData[NodeId].type;//得到源结点的类型  e.data.inthis.$nodeData
//		     var oldTxt = this.$nodeData[nodename].children(".td").innerHTML;
//			 alert(oldTxt);
////		   	e.data.inthis.setCurrentDa(id);//!@#0324
//		});
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			var oldTxt=this.innerHTML;
			var This=e.data.inthis;
			var id=$(this).parents(".GooFlow_item").attr("id");  //这个id其实是数据库中的nodename属性!@#
			var t=getElCoordinate(This.$workArea[0]);			
				//对oldTxt进行处理 若oldTxt后面包含#和数字，则取出数字，该数值为da_id，将其带入setCurrentDa 默认正常输入若有#号则非法
			if( oldTxt.indexOf("#")> -1  )	{
				var id_array  =  oldTxt.split("#");
				var da_id = id_array[id_array.length-1];
//				alert(da_id);
				e.data.inthis.setCurrentDa(da_id);
			}
...
//				if(e.button==2)return false;
//				//开始对文本输入进行非法处理判断
//				if(This.$textArea.val() == oldTxt || This.$textArea.val().indexOf("#") == -1 )//未修改 或 修改后不包含#
//				{ 
//						This.setName(This.$textArea.data("id"),This.$textArea.val(),"node");
//						This.$textArea.val("").removeData("id").hide();
//				}
//				else if(This.$textArea.val().indexOf("#")> -1  )//若修改后，包含# 则提示输入非法 并采用oldTxt
//				{
//					alert("输入非法，不能包含#字符");
//					This.setName(This.$textArea.data("id"),oldTxt,"node");
//					This.$textArea.val("").removeData("id").hide();
//				}
//			});
		});
	...
	//设置结点/连线/分组区域的文字信息
	setName:function(id,name,type){//name是数据库中name属性
		var oldName;
		if(type=="node"){//如果是结点
			///alert("!@#");
			if(!this.$nodeData[id])	return;
		...
		$.ajax({
			...
			//	if(para.dataFilter)	para.dataFilter(msg,"json");
				...
				for(var i=0;i<msg.length;i++){
					...
					var c = msg[i].color;
					var st = msg[i].style;
					var op = msg[i].opacity;
					var info = msg[i].daInfo;
					document.getElementById(key).style.background=c;
					document.getElementById(key).style.opacity=op;
					document.getElementById(key).getElementsByTagName('b')[0].className=st;
					document.getElementById(key).setAttribute('data-content',info);
				}
		//		This.clearData();
     	//		This.loadData(msg);
	...
	//	console.log(this.exportData());
		$.ajax({
			type:"post",
			url:"saveISPLayout.action",
			dataType:"json",
			data:{layout:this.exportData()},
			success: function(){
     			alert("保存成功！");
   			},
			error: function(XMLHttpRequest, textStatus, errorThrown){
	...
	getNodesId:function(para){ //!@#0414
		var This = this;
		$.ajax({
			type:"post",
			url:"getNodesId.action",
			dataType:"json",
			data:{},
			async:false,
			success:function(msg){
				This.loadSecondMonitor(para);//!@#0414
				for(var i = 0; i < msg.length; i ++) {
    				(function(name, id , info) {
    					$("#" + name).bind("click", function() {
             				 This.setCurrentDa(id);     				 
      	   			  		 });
    					$("#" + name+"_second1").bind("click", function() {
            				 This.setCurrentDa(id);     				 
     	   			  		 });
   	   			  		 var a = document.getElementById(name);
   	   			  		 var second_name = name+"_second";
  	   			  		 //alert(second_name);
   	   			  		 var b = document.getElementById(second_name);    	
   	   			  		 b.setAttribute("title","DA参数");
						 b.setAttribute("data-container","body");
						 b.setAttribute("data-toggle","popover");
						 b.setAttribute("data-placement","bottom");
						 b.setAttribute("data-content", info);				     
   	   			  		 a.setAttribute("title","DA参数");
						 a.setAttribute("data-container","body");
						 a.setAttribute("data-toggle","popover");
						 a.setAttribute("data-placement","bottom");
						 a.setAttribute("data-content", info);
  					})(msg[i].key, msg[i].daId, msg[i].daInfo);	
				}
			},
			error: function(XMLHttpRequest, textStatus, errorThrown){
				if(para.error)	para.error(textStatus,errorThrown);
    ...
    loadSecondMonitor:function(para){//!@#0414
		var This=this;
		$.ajax({
			type:"post",
			url:"loadSecondMonitor.action",
			dataType:"json",
			data:{},
			async:false,
			success: function(msg){
				if(para.dataFilter)	para.dataFilter(msg,"json");
     			This.loadSecondData(msg[0].layout);
   			},
			error: function(XMLHttpRequest, textStatus, errorThrown){
				if(para.error)	para.error(textStatus,errorThrown);
			}
...
//			this.addNode(i,data.nodes[i]);
			if(!property.haveTool){
				this.addPlanNode(i,data.nodes[i]);
			}
		}
		for(var j in data.lines)
			this.addLine(j,data.lines[j]);
	...
	addPlanNode:function(id,json)//id为nodename
	{
		if(this.onItemAdd!=null&&!this.onItemAdd(id,"node",json))return;
		if(this.$undoStack&&this.$editable){
			this.pushOper("delNode",[id]);
		...
		if(json.type.indexOf(" round")<0){
			...
			if(!json.left||json.left<0)json.left=0;
			var hack=0;
			var ExecuteStartTimeDec = 0;//代表实际开始时间的差减计划开始时间；无用
			var ExecuteEndTimeDec   = 0;//代表实际结束时间的差减计划结束时间；无用		
			if(navigator.userAgent.indexOf("8.0")!=-1)	hack=2;
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"'  id='"+id+"' style='opacity:0.5; top:"+json.top+"px;left:"+(json.left+ExecuteStartTimeDec)+"px'><table cellspacing='1' style='background-color:#b6f946;width:"+(json.width+ExecuteEndTimeDec-ExecuteStartTimeDec-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'></td><td>"+""+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
			if(json.type.indexOf(" mix")>-1)	this.$nodeDom[id].addClass("item_mix");//<table cellspacing='1' style='background-color:#80ff00;
		}
		else{
			json.width=24;json.height=24;
			json.width=json.width+ExecuteEndTimeDec-ExecuteStartTimeDec;
			this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='opacity:0.5;top:"+json.top+"px;left:"+(json.left+ExecuteStartTimeDec)+"px'><table cellspacing='0'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
		}
		var ua=navigator.userAgent.toLowerCase();
		if(ua.indexOf('msie')!=-1 && ua.indexOf('8.0')!=-1)
			this.$nodeDom[id].css("filter","progid:DXImageTransform.Microsoft.Shadow(color=#94AAC2,direction=135,strength=2)");
	...
	getCaRunningState:function(){
		var This = this;
		$.ajax({
			type:"post",
			url:"getCaRunningState.action",
			dataType:"json",
			data:{},
			async:false,
			success:function(msg){
	...
	getNodesIdProv:function(para){
		var This = this;
		$.ajax({
			type:"post",
			url:"getProvNodes.action",
			dataType:"json",
			data:{},
			async:false,
			success:function(msg){
				for(var i = 0; i < msg.length; i ++) {
    				(function(name, id , info) {
						 ...
						 a.setAttribute("data-placement","bottom");
						 a.setAttribute("data-content", info);
   	   			  		 //jia yige zifuchuan yonglai cunfang dade shuju
  					})(msg[i].key, msg[i].id, msg[i].info);
				}
			},
			error: function(XMLHttpRequest, textStatus, errorThrown){
				if(para.error)	para.error(textStatus,errorThrown);
	...
	setCurrentDa:function(daId){
	//alert("dandan"+daId);
	$.ajax({
			type:"post",
			url:"setCurrentDa.action",
			dataType:"json",
			data:{daId:daId},
			async:false,
			success:function(msg){
	...
	loadMonitor:function(para){
		var This=this;
	//	alert(1);
		$.ajax({
			type:"post",
			url:"loadMonitor.action",
			dataType:"json",
			data:{},
			async:false,
			success: function(msg){
	...
	loadCAforExecute:function(para){
		var This = this;
		$.ajax({
			type:"post",
			url:"getCASequencebyDA.action",
			dataType:"json",
			async:false,
			success: function(msg){
				console.log(msg);
	...
	reloadTitle:function(para){
		var This=this;
		//alert(1);
		$.ajax({
			type:"post",
			url:"reloadISPPic.action",
			dataType:"json",
			data:{},
			async:false,
			success: function(msg){
	...
	exportData:function(){
		...
		$.each(ret.nodes,function(key,value){
			var a = document.getElementById(key).getElementsByTagName('td')[1].innerHTML;
			ret.nodes[key].name = a;
		});
