...
//                              alert("fptypes"+fptypes.length);
// 							alert(tmp[2].id);
//                 			alert(tmp[2].name);
// 							alert(fptypes[4].id);
//                 			alert(fptypes[4].name);
...
//        				     for(var k= 0; k < DAs[i].length; k ++){
//                          			var flag = 1;
//                      			    var tmp_string;
//                      			    var z=0;			     
// 									for(var y=0;y<submenu_title.length;y++){
// 											  for(z = 0; z < tmp.length;z ++) {
// 					                                if(tmp[z].id == DAs[z][k].fightPostion){
// 					            	                        if(tmp[z].name == submenu_title[y])
// 																		flag=0;						
// 														   else 
// 														   				tmp_string = tmp[z].name;
// 														   }
// 					                            }
// 									}
// 									if(flag==1)
//        			           				 submenu_title.push(tmp[z].name);	   			  
//        			           	 }    			
                         ...
                         for(var j = 0; j < tmp.length; j ++) {
	                            	for(var k= 0; k < DAs[i].length; k ++){
		                                if(tmp[j].id == DAs[i][k].fightPostion)
		                                    submenu_title.push(tmp[j].name);	   			  
	                                    }
						}
						var hash=[];
						var result=[];
					    for (var m = 0, elem; (elem = submenu_title[m]) != null;m++) {
					        if (!hash[elem]) {
					            result.push(elem);
					            hash[elem] = true;
					        }
              	    	...
              	    	s+='</span><b class="arrow fa fa-angle-down"></b></a><b class="arrow"></b><ul class="submenu">';
			                    for(var j = 0; j < DAs[i].length; j ++) 
								         for(var t = 0; t < tmp.length; t++) {
							                                if(tmp[t].id == DAs[i][j].fightPostion&&tmp[t].name==result[k])
                                   			                    	{
            ...
            $(document).ready(function() {
                ...
                $("#delBtn").bind("click", function() {
    					...
    					if($("#delBtn").attr("data-daid")==-1)
    						return;
    					if(confirm("确定是否删除"+$("#daname").text()+"任务吗？")){
