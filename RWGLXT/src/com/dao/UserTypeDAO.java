...
public class UserTypeDAO extends BaseHibernateDAO {
	...
	public UserType findById(com.domain.UserTypeId id) {
		log.debug("getting UserType instance with id: " + id);
		try {
			UserType instance = (UserType) getSession().get(
					"com.domain.UserType", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(UserType instance) {
		...
		try {
			List results = getSession().createCriteria("com.domain.UserType")
