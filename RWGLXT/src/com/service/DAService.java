...
//		System.out.println( EventT.getLogic());
		...
		for (int i = 0; i < sameTrIdList.size(); i++) {
					Map map = new HashMap();
					map.put("attrid", sameTrIdList.get(i).getId().getAtrribute().getAtrId());
					map.put("symbs", sameTrIdList.get(i).getSymbol());
					map.put("vals", sameTrIdList.get(i).getValue());
					map.put("logics",logic_dandu[i]);
					u.add(map);
		}
		return u;
	}
