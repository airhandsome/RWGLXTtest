...
public class DefAction extends ActionSupport {
	...
	public String IspDef() {
		...
		ISPforCopy isp = sa.copyISP(ispId, new Integer(ActionContext.getContext().getSession().get("userid").toString()),time,name,desc);
		...
		ActionContext.getContext().getSession().put("ispDesc",desc);
		ActionContext.getContext().getSession().put("ispTime",time);
		return "Success";
	}
	...
	public String DaDef() throws IOException {
		...
		try {
			...
			id = insert.createDA(daId, planStart, planEnd, triggertype, list,
					caId, startattributeId, startsymbol, startvalue);
			ServletActionContext.getResponse().getWriter().print(id);
			return null;
		} catch (CloneNotSupportedException e) {
			// TODO Auto-generated catch block
